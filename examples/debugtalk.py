...
import jpype
from ate.common import last_name

from examples.config import jvmArg, jvmPath, test_private_key, test_encrypt_key, md5_sign_key, code_path
...
def get_request_msg(**kwargs):
    sign_string = kwargs.pop('data')
    encrypt_data = {}
    if 'encrypt_data' in kwargs.keys():
        encrypt_data = kwargs.pop('encrypt_data')
        encrypt_data = encrypt_rsa(encrypt_data)
        for key, value in encrypt_data.items():
            sign_string[key] = value  # 添加加密后的key-value
            encrypt_data[key] = quote(value, safe=';')  # encode, 除去;
    ...
    if sign_type.upper() == 'RSA':
        signed = get_sign_rsa(input_string)
    elif sign_type.upper() == 'MD5':
        signed = get_sign_md5(input_string)

    signed = quote(signed, safe=';')  # encode sign
    ...
    # quote就是java的urlencode
    try:
        sign_string['notify_url'] = quote(sign_string.pop('notify_url'), safe=',')
        sign_string['return_url'] = quote(sign_string.pop('return_url'), safe=',')
    except:
        pass
    sign_string['sign'] = signed
    kwargs.setdefault('data', sign_string)
    return kwargs
...
def get_sign_rsa(input_string):
    ...
    sign_util = jpype.JClass("com.common.SignUtil")
    rsa = sign_util()

    key = test_private_key
    # if get_value('env') == 'PRO':
...
def get_sign_md5(input_string):
    ...
    md5_string = input_string + md5_sign_key
    sign = md5.getMD5(md5_string)
    return sign
...
def encrypt_rsa(dict):
    ...
    # if get_value('env') == 'PRO':
    #     encrypt_key = verify_encrypt_key
